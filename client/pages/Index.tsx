  ...
  const featureTiles: Array<React.ReactNode> = [
    ...
    >
      <CardHeader className="p-4">
        ...
        <CardTitle className="text-sm">
          <a href="/first-aid">First Aid Guide</a>
        </CardTitle>
        <CardDescription className="hidden sm:block">
          CPR, bleeding, choking
        </CardDescription>
      </CardHeader>
    </Card>,
    <ChecklistDialog
    ...
    <Card
      ...
      onClick={() =>
        toast({
          title: "Wellness tips",
          description: "Hydrate, sleep 7–9h, regular movement, balanced diet.",
        })
      }
      className="group cursor-pointer transition-colors hover:bg-accent/30"
    >
  ...
  return (
    <main className="min-h-screen bg-background">
          ...
          <div className="mt-4 text-xs text-muted-foreground">
            {loc ? (
              <span>
                ...
                Local emergency: {emergency.main}
              </span>
            ) : (
              <button
      ...
      <section className="container py-8">
        <h2 className="font-brand text-lg font-semibold bg-gradient-to-r from-brand to-accent bg-clip-text text-transparent">
          Mobile wellbeing
        </h2>
        <p className="mt-1 text-xs text-muted-foreground">
        ...
        <div className="container py-10">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {[
              {
                name: "Riya",
                text: "The SOS and hospital finder helped me during a road incident.",
              },
              {
                name: "Karan",
                text: "Clear first‑aid steps gave me confidence in an emergency.",
              },
            ].map((r) => (
              <div
                key={r.name}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-red-600/95 p-6 text-white animate-pulse">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm">
              Show this screen to attract attention
            </p>
            <div className="mt-6">
              <Button variant="secondary" onClick={() => setVisualAid(false)}>
                Close
              </Button>
            </div>
          </div>
      ...
      <section id="cta" className="border-t">
        <div className="container py-12 text-center">
          <p className="text-sm text-muted-foreground">
            In an emergency, call your local number immediately.
          </p>
        </div>
      </section>
    </main>
